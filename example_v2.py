...
def write_into_snapshot(ebs, snapshot_id, data, thread_num):
    ...
    for block_index in range(TOTAL_512KB_BLOCKS):
        if block_index % (NUM_THREADS) == thread_num:
            total_blocks = total_blocks + 1
            rsp = ebs_put_block_on_snapshot(ebs, snapshot_id, block_index, data, BLOCK_SIZE_512KB,  base64_encoded_cksum, 'SHA256')
            print("[thread = "+str(thread_num)+"] [block_index]" + str(block_index) +"] written\n")
    print("[thread = "+ str(thread_num) +"]" + " total_blocks ="+ str(total_blocks))
...
snapshot_id = ebs_start_snapshot(ebs, EBS_SNAPSHOT_SIZE, "Subodh_EBS_Direct_v2")
#ebs_list_snapshot_blocks(ebs, snapshot_id)
data = read_dummy_data()

start_time = time.time()
writer_thread_list = []
for thread_num in range(NUM_THREADS):
    t = threading.Thread(target = write_into_snapshot, args=(ebs,snapshot_id,
data, thread_num))
    t.start()
    writer_thread_list.append(t)

for t in writer_thread_list:
    t.join()
